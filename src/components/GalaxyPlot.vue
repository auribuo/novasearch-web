...
if (fetcher) {
  ...
  onMounted(async () => {
    ...
    plot.on("plotly_autosize", () => {
      if (isDark.value) {
        darkenPlotly(plotlyRoot);
      }
    });
    plot.on("plotly_redraw", () => {
      if (isDark.value) {
        darkenPlotly(plotlyRoot);
      }
    });
...
<template>
  <div class="p-3 hidden md:flex w-full">
    <div class="w-full" id="plot" ref="plotlyRoot"></div>
  </div>
  <div class="md:hidden">
    <n-space justify="center">
      <n-result status="error" title="Error" description="Please open the plot on a wider screen"></n-result>
    </n-space>
  </div>
</template>